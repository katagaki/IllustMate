                    ...
                    if !isSelecting {
                        ...
                        if pic.containingAlbumID != nil {
                            Button("Shared.SetAsCover", systemImage: "photo") {
                                Task {
                                    await actor.setAsAlbumCover(for: pic.id)
                                }
                            }
                        }
                        Divider()
...
struct PicShareable: Transferable {
    ...
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(exportedContentType: .image) { shareable in
            await actor.imageData(forPicWithID: shareable.picID) ?? Data()
        }
    }
}
...
struct PicPreview: View {
    ...
    var body: some View {
        ...
        .task {
            if let thumbData = await actor.thumbnailData(forPicWithID: picID),
